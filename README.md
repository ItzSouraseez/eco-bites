...
#### `scans`
User's scan history (camera + search):
...
#### `pendingProducts`
Custom dataset contributions awaiting admin approval.

#### `dietLogs/{userId}/{date}/{entryId}`
Daily consumption logs captured from scanner prompts.

#### `userGoals/{userId}`
User-defined calorie/macro goals for the diet dashboard.

#### `dietPlans/{userId}/latest`
Latest personalized plan generated by Gemini.

#### `reminders/{userId}/{reminderId}`
Scheduled diet/hydration reminders.
